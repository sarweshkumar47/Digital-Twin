...
print("========> Debugging: monitoring.py start() <=========")
debug_vars = []
...
def parse_st(st_file):
    print("=========> Debugging: monitoring.py parse_st filename: {} <=========".format(st_file))
    global debug_vars
    # filepath = './st_files/' + st_file
    filepath = '/home/pi/digital-twin/openPLC_v3/webserver/st_files/' + st_file
...
def cleanup():
    del debug_vars[:]
    print("=======> Debugging: monitoring.py, cleanup() <=======")
...
def modbus_monitor():
    ...
    if (len(debug_vars) > 0):
        print("==========> Debugging: monitoring.py, modbus_monitor(), debug_vars > 0 <=========")
    else:
        print("==========> Debugging: monitoring.py, modbus_monitor(), debug_vars is None <=========")

    for debug_data in debug_vars:
        print("==========> Debugging: monitoring.py, modbus_monitor(), debug_data: {} <=========".format(debug_data))
        ...
        if (debug_data.location.find('IX')) > 0:
            ...
            # print ("==========> Debugging: monitoring.py, modbus_monitor(), mb_address: " + str(mb_address))
            ...
            if (debug_data.type == 'SINT') or (debug_data.type == 'INT') or (debug_data.type == 'DINT') or (
                    debug_data.type == 'LINT'):
                # signed integer
                float_pack = pack('>HHHH', result.registers[0], result.registers[1], result.registers[2],
                                  result.registers[3])
                debug_data.value = unpack('>q', float_pack)[0]

            if (debug_data.type == 'USINT') or (debug_data.type == 'UINT') or (debug_data.type == 'UDINT') or (
                    debug_data.type == 'ULINT'):
                # unsigned integer
                float_pack = pack('>HHHH', result.registers[0], result.registers[1], result.registers[2],
                                  result.registers[3])
                debug_data.value = unpack('>Q', float_pack)[0]
        ...
        print("==========> Debugging: monitoring.py, modbus_monitor(), eol, debug_data.name: {} <=========".format(
            debug_data.name))
        print("==========> Debugging: monitoring.py, modbus_monitor(), eol, debug_data.location: {} <=========".format(
            debug_data.location))
        print("==========> Debugging: monitoring.py, modbus_monitor(), eol, debug_data.value: {} <=========".format(
            debug_data.value))

    if (monitor_active == True):
        print("########### > Debugging: monitoring.py threading timer() start time stamp: {}".format(
...
def start_monitor():
    ...
    if (monitor_active != True):
        ...
        print("==========> Debugging: monitoring.py, start_monitor(), mb_client: {} <=========".format(mb_client))
